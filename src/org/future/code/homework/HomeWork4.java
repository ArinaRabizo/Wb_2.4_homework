...
public class HomeWork4 {
    ...
    public static List<String> correctedList(List<String> stringList) {
        ...
        for (int i = 0; i < stringList.size(); i++) {
            lower.add(i, stringList.get(i).toLowerCase());
        }
        ArrayList<String> lower1 = new ArrayList<>();
        System.out.println(lower);
        for (int i = 0; i < lower.size(); i++) {
            if (stringList.get(i).equals(lower.get(i))) {
                lower1.add(lower.get(i));
            }
        }
        return lower1;
    ...
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
     if(shopCart.get("яблоки") > 300) {
         shopCart.remove("яблоки");
     }
        if(shopCart.get("груши") > 300) {
            shopCart.remove("груши");
        }
        if(shopCart.get("лук") > 300) {
            shopCart.remove("лук");
        }
        if(shopCart.get("свекла") > 300) {
            shopCart.remove("свекла");
        }
        if(shopCart.get("картошка") > 300) {
            shopCart.remove("картошка");
        }
        if(shopCart.get("чеснок") > 300) {
            shopCart.remove("чеснок");
        }
        if(shopCart.get("ягоды") > 300) {
            shopCart.remove("ягоды");
        }
